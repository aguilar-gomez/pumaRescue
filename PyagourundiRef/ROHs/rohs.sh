...
KEEP=unrelatedIndv
bcftools roh -e ${KEEP} -G 30 -Orz -o ${VCF}_bcftoolsROH_unrelated.txt.gz ${VCF}

# Reformat output
zcat ${VCF}_bcftoolsROH_unrelated.txt.gz | tail -n+4 \
| sed 's/# //g' | sed -E 's/\[[0-9]\]//g' | sed 's/ (bp)//g' \
| sed 's/ (average fwd-bwd phred score)//g' \
| tr ' ' '_'> ${VCF}_bcftoolsROH_unrelated.txt

#Total sequence analyzed, smaller genome: 2,415,546,177
DATA=${VCF}_bcftoolsROH.txt_unrelated.gz
while read -r SAMPLE ; do 
zcat ${DATA} \
| awk -v s=${SAMPLE} 'BEGIN{sum=0}{if ($2==s && $6>=1e6){sum+=$6; num+=1}}END{printf "%s\t%s\t%s\t%s\n", s, sum/2415546177, num, sum/num}'
done < samples.list > ROHS_pumas_unrelated.totalgenome
...
FILE=pumas
plink --vcf puma_simplePASS_variants_all.vcf.gz --make-bed --out $FILE --allow-extra-chr --keep-allele-order --double-id 

# Identify ROH and reformat output
plink --bfile ${FILE} --out ${FILE}_ROH --homozyg --allow-extra-chr 
#--homozyg: Scan complete, found 10481 ROH.
