...
out=$(basename $bamfile .bam)
{ time bcftools mpileup -C50 -O u -f GCF_014898765.1_PumYag_genomic.fna $bamfile 2> fq/${out}_stderr.txt  | bcftools call -c - | vcfutils.pl vcf2fq -d $mindep -D $maxdep | gzip > fq/${out}withROH.fq.gz ; } 2> fq/bam2fq_${out}withROH.time 
echo "Waiting for bam2fq to finish"
wait
echo "Running fq2psmcfa"
{ time fq2psmcfa -q 30 -s 100 fq/${out}withROH.fq.gz > psmcIN/${out}withROH.psmcfa ; } 2> psmcIN/fq2psmc_${out}withROH.time
